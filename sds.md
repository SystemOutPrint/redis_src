...
## 0x03 字符串编码
int、embstr、raw<br/>
int编码用于long型能表示的整型数据，都可以使用该编码格式。<br/>
embstr编码只限于小于等于39字节的字符串，64 - 39 = 16(redisObject) + 8(sdshdr) + 1<br/>
raw编码用于大于39字节的字符串。

## 0x04 字符串共享
int编码的字符串可以共享，通过redisObject的refCount来实现。redis会在启动的时候缓存0-9999的数字用来共享。<br/>
